...
| 2026-06-26 | Added chat 422 fix session ($1.00): fixed #22 (gr.Chatbot type='messages' — every 2nd chat/quick-action message 422'd because component defaulted to tuples while handlers use messages format); AST regression guard; v0.6.2 release; cumulative 15 sessions, ~$38.05 |
| 2026-06-26 | Added App Insights session ($1.00): instrumented Gradio UI with azure-monitor-opentelemetry (#23) — guarded configure_azure_monitor init (no-op without conn string), OTEL_SERVICE_NAME=deepseismic2-ui cloud role, logger.exception on agent errors; +4 AST/source tests; v0.6.3 release; cumulative 16 sessions, ~$39.05 |
| 2026-06-26 | Added run-id + WAF triage session ($1.50): HAR proved the 2nd-chat-message failure is an Azure Front Door WAF 403 false positive (infra#18, no app change); fixed run-id prefix resolution so the UI's 8-char id resolves via in-memory + catalog (chat-after-reload 404), surfaced full run id in UI; +4 tests; v0.6.4 release; cumulative 17 sessions, ~$40.55 |
| 2026-06-26 | Added F3 cross-survey ingest scaffolding session ($1.00): for #24 (synthetic-trained model over-predicts on real data → cross-survey F3→Volve blind eval), created scripts/download_f3.py (real F3 acquisition contract + format-proxy generator, 28 Hz wavelet vs Volve's 35 Hz) and verified the full proxy ingest pipeline (segy_to_zarr + fault-label rasteriser) produces co-registered amplitude+label volumes; +6 ingest tests (343 total); no app release (data acquisition + training remain); cumulative 18 sessions, ~$41.55 |
